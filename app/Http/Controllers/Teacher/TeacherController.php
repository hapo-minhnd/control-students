...
class TeacherController extends Controller
{
  ...
  public function LoginTeacher(LoginTeacher $request)
  {
    ...
    if (Auth::guard('teacher')->attempt(['code_teacher' => $code_teacher, 'password' => $password, 'active' => 1])) {
      ...
      if (Auth::guard('teacher')->attempt(['code_teacher' => $code_teacher, 'password' => $password])) {
        dd('pls check mail');
      } else {
  ...
  public function pickClass(Request $request, $id)
  {
    ...
    return view('teacher.update_score', ['pointSubjects' => $pointSubjects , 'id' => $id]);
  }
  ...
  public function indexPick(request $request)
  {
    ...
    $classes = ClassStudent::where('code_teacher', $code_teacher)->where('semester', $semester)->get();
    return view('teacher.pick_class', ['classes' => $classes , 'semester' =>$semester]);
  }
  ...
  public function pickSemester(Request $request)
  {
    ...
    return view('teacher.update_class', ['ClassStudents' => $ClassStudents, 'semester' => $semester]);
  }
  ...
  public function infoStudent(Request $request)
  {
    if ($request->has('code_student')) {
      ...
      $students = Student::where('code_student', 'like', "%{$cs}%")->orWhere('code_class', 'like',
          "%{$cs}%")->paginate(5);
      return view('teacher.info_student', ['students' => $students]);
    } else {
